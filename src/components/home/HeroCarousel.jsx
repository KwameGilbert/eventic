...
const HeroCarousel = ({ items }) => {
  ...
  const isAward =
    currentItem.type === "award" || currentItem.ceremony_date !== undefined;

  // Determine image URL
  const imageUrl =
    currentItem.banner_image || currentItem.image || "/placeholder.jpg";
  ...
  const getDisplayData = () => {
    if (isAward) {
      return {
        icon: Trophy,
        ctaText: currentItem.is_voting_open ? "VOTE NOW" : "VIEW DETAILS",
        metadata: [
          {
            icon: MapPin,
          ...
          },
          {
            ...
            text: currentItem.ceremony_date
              ? new Date(currentItem.ceremony_date).toLocaleDateString(
                  "en-US",
                  {
                    month: "long",
                    day: "numeric",
      ...
      return {
        ...
        metadata: [
          ...
          },
          {
            icon: Clock,
            text:
              currentItem.date || currentItem.start_time
                ? currentItem.date ||
                  new Date(currentItem.start_time).toLocaleDateString("en-US", {
                    month: "long",
                    day: "numeric",
                    year: "numeric",
                  })
          ...
          <div className="space-y-3 mb-8">
            {displayData.metadata.map((meta, idx) => {
              ...
              return (
                ...
                >
                  <MetaIcon size={20} className="shrink-0" />
                  <span>{meta.text}</span>
                </div>
              );
            })}
