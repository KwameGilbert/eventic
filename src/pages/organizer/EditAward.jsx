...
const EditAward = () => {
    ...
    const fetchAward = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          // Parse ceremony date and time
          let ceremonyDate = "";
          let ceremonyTime = "";
          if (award.ceremony_date) {
            const ceremonyDateTime = new Date(award.ceremony_date);
            ceremonyDate = ceremonyDateTime.toISOString().split("T")[0];
            ceremonyTime = ceremonyDateTime.toTimeString().slice(0, 5);
          }

          // Parse voting dates
          const votingStart = award.voting_start
            ? new Date(award.voting_start).toISOString().slice(0, 16)
            : "";
          const votingEnd = award.voting_end
            ? new Date(award.voting_end).toISOString().slice(0, 16)
            : "";
        ...
        setLoadError(
          err.message || "An error occurred while loading award details",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (status = null) => {
    ...
    try {
      ...
      const ceremonyDateTime =
        awardData.ceremonyDate && awardData.ceremonyTime
      ...
      // Add banner image if a new one was uploaded
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={() => navigate(`/organizer/awards/${id}`)}
          className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
        >
          <ArrowLeft size={20} className="text-gray-600" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Award</h1>
          <p className="text-gray-500 mt-1">Update award information</p>
        </div>
      </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {awardData.ceremonyDate
                      ? new Date(awardData.ceremonyDate).toLocaleDateString()
                      : "No date set"}
                  </p>
            ...
            <div className="space-y-3">
              ...
              <Button
                ...
                className="w-full gap-2"
                disabled={
                  isSubmitting || !awardData.title || !awardData.ceremonyDate
                }
              >
                {isSubmitting ? (
                  <>
