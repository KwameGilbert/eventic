...
const Awards = () => {
  ...
  const getStatusStyle = (status) => {
    switch (status.toLowerCase()) {
      ...
      case "pending":
        return "default"; // Blue (info/default)
      case "draft":
        return "warning"; // Yellow/Orange
      case "voting open":
        return "info"; // Blue (computed from voting dates)
      case "completed":
        ...
        <table className="w-full table-fixed">
          <thead>
            <tr className="border-b border-gray-100">
              <th className="text-left py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[210px]">
                Award
              </th>
              <th className="text-left py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[120px]">
                Ceremony Date
              </th>
              <th className="text-left py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[120px]">
                Venue
              </th>
              <th className="text-left py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[100px]">
                Status
              </th>
              <th className="text-left py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[100px]">
                Categories
              ...
              <th className="text-right py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[120px]">
                Actions
              </th>
            </tr>
          </thead>
          <tbody>
            {filteredAwards.map((award) => (
              <tr
                key={award.id}
                className={cn(
                  "border-b border-gray-50 hover:bg-gray-50 transition-colors group cursor-pointer",
                )}
                onClick={() => navigate(`/organizer/awards/${award.id}`)}
              >
                {/* Award */}
                <td className="py-4 px-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p
                        className="font-medium text-gray-900 truncate group-hover:text-orange-500 transition-colors"
                        title={award.title}
                      >
                        {award.title}
                      </p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage and monitor all your award events
          </p>
        </div>
