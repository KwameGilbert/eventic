...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const awards =
          awardsResponse?.data?.awards || awardsResponse?.awards || [];

        // Fetch upcoming awards
        const upcomingAwardsResponse = await awardService.getUpcoming({
          per_page: 12,
        });
        const upcomingAwardsData =
          upcomingAwardsResponse?.data?.awards ||
          upcomingAwardsResponse?.awards ||
          [];

        // Fetch featured events for carousel
        const featuredEventsResponse = await eventService.getFeatured(5);
        const featuredEventsData =
          featuredEventsResponse?.data || featuredEventsResponse || [];

        // Fetch upcoming events
        const upcomingResponse = await eventService.getUpcoming({
          per_page: 12,
        });
        const upcoming =
          upcomingResponse?.data?.events || upcomingResponse?.events || [];

        setFeaturedAwards(Array.isArray(awards) ? awards : []);
        setUpcomingAwards(
          Array.isArray(upcomingAwardsData) ? upcomingAwardsData : [],
        );
        setFeaturedEvents(
          Array.isArray(featuredEventsData) ? featuredEventsData : [],
        );
  ...
  return (
    <div className="min-h-screen">
      <SEO
        title="Home"
        description="Welcome to Eventic - Africa's premier awards and events voting platform. Discover, vote, and celebrate excellence."
      />
      {/* Error Banner */}
      {error && (
