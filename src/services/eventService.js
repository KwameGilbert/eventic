    ...
    if (tickets && tickets.length > 0) {
      formData.append(
        ...
        JSON.stringify(
          tickets.map((ticket) => ({
            ...
            promoPrice: parseFloat(ticket.sale_price) || 0,
            quantity: parseInt(ticket.quantity) || 0,
            maxPerOrder: parseInt(ticket.max_per_user) || 10,
            description: ticket.description || "",
            saleStartDate: ticket.sale_start || null,
            saleEndDate: ticket.sale_end || null,
            status: ticket.status || "active",
            ...(ticket.dynamic_fee !== undefined && {
              dynamic_fee: parseFloat(ticket.dynamic_fee) || 0,
            }),
          })),
        ),
      );
      ...
      tickets.forEach((ticket, index) => {
        if (ticket.ticket_image) {
          formData.append(`ticket_image_${index}`, ticket.ticket_image);
        }
      });
    }
        ...
        JSON.stringify(
          tickets.map((ticket) => ({
            id:
              ...
              (!isNaN(ticket.id) && !ticket.id.toString().startsWith("new-"))
                ? parseInt(ticket.id)
                : undefined,
            name: ticket.name,
            price: parseFloat(ticket.price) || 0,
