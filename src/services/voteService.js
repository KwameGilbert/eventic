...
const voteService = {
   ...
   * @param {string|Object} data - Payment reference or token object
   * @returns {Promise<Object>} Confirmation result
   */
  confirmPayment: async (data) => {
    const payload = typeof data === "string" ? { reference: data } : data;
    const response = await api.post("/votes/confirm", payload);
    return response;
  },
  ...
  getByCategory: async (categoryId, params = {}) => {
    const response = await api.get(`/votes/award-categories/${categoryId}`, {
      params,
    });
    return response;
  },
  ...
  getLeaderboard: async (categoryId) => {
    const response = await api.get(
      `/votes/award-categories/${categoryId}/leaderboard`,
    );
    return response;
  },
