...
const orderService = {
    ...
    try {
      ...
      if (items.length === 0) {
        return {
          success: false,
          error:
            "No valid ticket items found in cart. Make sure you are using events from the API (with ticket type IDs), not mock data.",
        };
      }
  ...
  calculateTotal: (cartItems) => {
    let subtotal = 0; // This will be the effective subtotal (base + dynamic markup)
    let totalDynamicMarkup = 0;
    let itemCount = 0;
      ...
      Object.entries(item.tickets || {}).forEach(([ticketName, qty]) => {
        if (qty > 0) {
          ...
          if (ticketType) {
            ...
            const dynamicFeePercent = parseFloat(
              ticketType.dynamicFeePercent || ticketType.dynamic_fee || 0,
            );

            // Effective price per ticket (base + markup)
            const effectivePrice = ticketType.effectivePrice
              ? parseFloat(ticketType.effectivePrice)
              : basePrice + basePrice * (dynamicFeePercent / 100);

            const markupAmount = effectivePrice - basePrice;

            subtotal += effectivePrice * qty;
            totalDynamicMarkup += markupAmount * qty;
          }
